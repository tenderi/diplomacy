...
All live in `/root/diplomacy/.env` on the VPS (mode 600).
...
3. On the VPS: `DOMAIN=<name>` in `/root/diplomacy/.env`, then
   `./upgrade.sh`.
...
cd /root/diplomacy && ./backup.sh   # expect "off-host copy done"
```
