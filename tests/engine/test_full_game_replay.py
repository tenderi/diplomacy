...
through the server and diffing every phase; nothing here did until the
pre-deletion audit of `old_implementation/` (Track W).
...
def test_the_recorded_game_is_actually_worth_replaying() -> None:
    ...
    # No retreat-phase check: `simple_ai` never issues a Support order (see
    # `simple_ai.py`), so no move can ever exceed strength 1 and no unit can
    # ever be dislodged -- a retreat phase is structurally impossible here
    # regardless of seed or phase count, not a property of this recording.
