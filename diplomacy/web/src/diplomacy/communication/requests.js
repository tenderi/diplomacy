...
export const REQUESTS = {
    ...
    // Check if a request is phase-dependent
    isPhaseDependent: (name) => {
        ...
        return (
            ...
            (model.phase_dependent ?? true) // Defaults to true if not explicitly defined
        );
    },

    // Get the request level
    getLevel: (name) => {
        const model = REQUESTS.models[name];
        if (!model) {
            throw new Error(`Unknown request name: ${name}`);
    ...
    create: (name, parameters = {}) => {
        ...
        const models = [
            ...
            ...(modelDefinition.level === STRINGS.GAME
                ? [REQUESTS.abstract.game, REQUESTS.abstract.channel]
                : modelDefinition.level === STRINGS.CHANNEL
        ...
        // Assign provided parameters to the request
