...
- `database.py` / `database_service.py` — SQLAlchemy models plus the DAL for everything **not** engine-coupled (users, players, messages, channels, tournaments). `game_repo.py` — `GameRepo`, the only persistence path for game state: `state_json` (a serialized `GameState`), `pending_orders`, `last_resolution`, `order_history`, `resolution_history`. There are no relational board tables: a game's board is its `state_json`, and the per-turn boards are rows of `map_snapshots`.
