...
class DataModelCompliantDemoGame:
    ...
    def generate_and_save_map(self, game_state: GameState, filename: str):
        ...
        try:
            ...
            # Get supply center control data for proper coloring
            supply_center_control = game_state.get_supply_centers()
            
            img_bytes = Map.render_board_png(svg_path, units, output_path=map_path, phase_info=phase_info, supply_center_control=supply_center_control)
    ...
    def submit_orders(self, power: str, orders: List[Order]) -> bool:
        ...
        try:
            ...
            # Submit all orders for this power in a single command
            all_orders_string = ' '.join(order_strings)
            result = self.server.process_command(f"SET_ORDERS {self.game_id} {power} {all_orders_string}")
            if result.get("status") != "ok":
                print(f"❌ Failed to submit orders for {power}: {result.get('message', 'Unknown error')}")
                return False
    ...
    def run_demo_game(self):
        ...
        # Check if retreat phase is needed
        if game_state.current_phase == "Retreat":
            self.print_header("SPRING 1901 RETREAT")
            retreats = self.generate_dynamic_orders(game_state)
            
            print(f"\n🤖 BOT COMMANDS:")
            for power, power_retreats in retreats.items():
                if power_retreats:
                    retreat_strings = [str(retreat) for retreat in power_retreats]
                    print(f"   /orders {self.game_id} {power} {' '.join(retreat_strings)}")
                    self.submit_orders(power, power_retreats)
                else:
                    print(f"   {power}: No retreats needed")
            ...
            # Generate map after retreats
            self.generate_and_save_map(game_state, "compliant_demo_05_spring_retreat.png")
        else:
            print(f"\n✅ No retreats needed - proceeding to Autumn Movement")
        ...
        # Check if retreat phase is needed
        if game_state.current_phase == "Retreat":
            self.print_header("AUTUMN 1901 RETREAT")
            retreats = self.generate_dynamic_orders(game_state)
            ...
            print(f"\n✅ No retreats needed - proceeding to Builds")
        ...
        self.generate_and_save_map(game_state, "compliant_demo_10_autumn_builds.png")
