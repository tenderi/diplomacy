...
def test_list_games_and_state_import_ok():
	...
	email = f"test_list_{int(time.time() * 1000)}@example.com"
	...
	# Create a game
	r_create = client.post("/games/create", json={"map_name": "standard"}, headers=headers)
	if r_create.status_code != 200:
		error_detail = r_create.json().get("detail", "Unknown error")
