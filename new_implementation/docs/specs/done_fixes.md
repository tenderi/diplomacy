# Done Fixes — Completed Work Archive (Tracks A–E)

> **This file is the historical record, not the tracker.** For what to work on next, read
> [`fix_plan.md`](fix_plan.md) — it holds only open work. Nothing in this file is open.
>
> **Why keep it:** every section below records not just *what* was done but the findings,
> rejected alternatives, and traps hit along the way. Several of those findings are still
> load-bearing for future work (`format_order` renders fleets as `A`; naive-UTC timestamp
> columns; frontend tests need an explicit `<Route>`; rendering changes must be validated by
> comparing PNG bytes). Those are summarised in `fix_plan.md`'s "Carried-over facts" so a
> reader never has to come here to avoid a known trap — but the full reasoning lives here.
>
> **Maintenance contract:**
> - When a track in `fix_plan.md` completes, move its section here **verbatim** (findings,
>   decisions, and verification evidence included) and delete it from the tracker.
> - Never edit a completed section to reflect later changes. If a decision recorded here was
>   later reversed, say so in the *new* work's section — this file is append-only history.
> - Do not add open tasks here. An unchecked box in this file is a bug.

## Index

| Track | What it was | Landed |
| --- | --- | --- |
| Engine rewrite (M0–M7) | The adjudicator rewrite itself | `v2.7.15`, PR #5 |
| **A — Finish the Port** | Port the layers that expose the engine (PR1–PR6) | `v2.7.17`–`v2.7.25` |
| **B — Post-rewrite cleanup** | Dead code + rendering quality (V0–V5) | `v2.7.19`–`v2.7.31` |
...
## Status at archival (2026-07-30, `v2.7.56`)
...
- **Archived:** 2026-07-30 at `v2.7.56`. **Track E complete** (E1–E4, `v2.7.51`–`v2.7.55`,
  PRs #44/#45/#46/#48 merged). `main` green, no open PRs, no stale branches. At the moment of
  archival the only open items were Track A's manual end-to-end check and Track E's five
  unscheduled gaps — both now carried in [`fix_plan.md`](fix_plan.md) as Tracks F and G.
...
## Final acceptance — manual end-to-end check → **MOVED, still open**

The five manual steps (bot starts, game created and mapped in Telegram, deliberate
dislodgement, retreat phase in both clients, build phase in both clients) were never run.
They need a live bot token and a human at a Telegram client, so they were never delegable to
an agent. **They live on as Track F in [`fix_plan.md`](fix_plan.md)** — do not re-add them
here until they are actually checked off.
...
## Known gaps recorded during Track E → **MOVED, still open**

Track E's audits recorded five unscheduled gaps (manual-`/processturn` notification silence,
unbounded support keyboards, the in-memory `WAITING_LIST`, no full province names, two API
warts). They were re-verified against the code on 2026-07-30 and are now **Track G in
[`fix_plan.md`](fix_plan.md)**, with file/line evidence and per-task steps.

---

## Definition of done (Tracks A–E)

Track A's manual acceptance criterion is deliberately absent — it is the one item from these
tracks that is still open. See Track F in [`fix_plan.md`](fix_plan.md).
...
- [x] **Decided 2026-09-24: strict** (maintainer). Landed as `v2.7.103`:
      `help_text.REJECTED_ORDER_FORMS` (`PAR H`, `RETREAT F IRI - MAO`, `REMOVE F LIV`,
      `A LON - NTH - BEL`) are shown in the shared order-format notes as "not accepted", each
      next to its canonical spelling, and `tests/test_bot_help_text.py` asserts every one
      still fails to parse while every other example parses. (The old README's `IRO` was
      never a province code; the examples use `IRI`.)
...
- [x] W7: decided strict and landed (`v2.7.103`).
