...
class GameService:
    ...
    def create_game(
        ...
        ``phase_length_seconds`` is how long each phase lasts before the deadline
        scheduler processes it: ``None`` uses the 24 h default, ``0`` means the
        game has no automatic deadline at all. See ``GameModel``.
