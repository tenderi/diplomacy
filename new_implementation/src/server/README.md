...
`POST /users/persistent_register` (bot secret); `GET /users/{telegram_id}/games` (bot secret,
or a Bearer user reading their own linked id); `GET /users/me/games` (Bearer).
...
`players`; `deadline` (GET and POST; a POST carrying `telegram_id` is checked for membership
and announced to the other players); `process_turn`; `history/{turn}`; `snapshot` (any
authenticated caller), `snapshots`, and `restore/{snapshot_id}` (**`X-Admin-Token` only** —
it rewinds the game; players are told); `draw_vote`, `draw_vote_status`, `concede`.
