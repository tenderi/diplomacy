...
def _phase_code_for(game_id: str, numeric_game_id: int, sent_at: datetime) -> Optional[str]:
    """The phase a message written at ``sent_at`` belongs to (K4).

    Normally the game's current phase. For a message the bot queued while the
    home server was unreachable, ``sent_at`` can predate the current phase, and
    the snapshot trail says which phase was live then (``get_phase_code_at``) --
    so a delayed message is filed under the phase the player was looking at, not
        ...
        result = [
            {
                ...
                # predating K4. Lets a client group a game log by phase.
