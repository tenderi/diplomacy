...
class GameModel(Base):
    ...
    # ``{turn_number_str: resolution_to_dict(...)}``, appended by
    # ``GameRepo.save_state`` on every processed turn. ``last_resolution`` holds
    # only the newest one and is overwritten each turn, so before this column
    # existed "what happened in turn 3" was unanswerable once turn 4 had run --
    # the orders survived (``order_history``) but their outcomes did not (K4).
    resolution_history = Column(JSON, nullable=True)
    deadline = Column(DateTime, nullable=True)  # Optional deadline for turn processing
    # How long each phase lasts, in seconds. Re-applied after **every** processed
    # turn by ``api.shared.after_turn_processed``; ``deadline`` above is just the
    # current phase's expiry computed from it.
    #
    # ``NULL`` means "use ``DEFAULT_PHASE_LENGTH_SECONDS`` (24 h)", which is what
    # every game created before this column existed gets, and what the manual
    # process-turn route hardcoded before K2. ``0`` means **no** automatic
    # deadline: the turn only advances when somebody processes it by hand
    # (the old engine's ``NO_DEADLINE``).
...
class MessageModel(Base):
    ...
    # The game phase this message was *written* in (``S1901M``), so a game log can
    # answer "what was said during F1902M" -- the old engine's ``Message`` carried
    # this and the port dropped it (K4). Resolved from ``timestamp`` (which is the
    # composed-at time, not the arrival time -- see ``api.client_timestamp``), so a
    # message the bot queued across a phase boundary is attributed to the phase the
    # player was actually looking at. NULL for messages written before this column
    # existed, and whenever the phase cannot be determined.
